...
creditcard = Spree::CreditCard.find_or_create_by(:cc_type => 'visa', :month => 12, :year => 2.years.from_now.year, :last_digits => '1111',
                                      :name => 'Sean Schofield', :gateway_customer_profile_id => 'BGS-1234')

Spree::Order.all.each_with_index do |order, index|
  order.update_with_updater!
  payment = order.payments.find_or_create_by(:amount => order.total, :source => creditcard.clone, :payment_method => method)
